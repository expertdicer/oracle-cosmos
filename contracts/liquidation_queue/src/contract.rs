...
use cosmwasm_std::{
    attr, from_binary, to_binary, Binary, Deps, DepsMut, Env, MessageInfo, 
    InitResponse, HandleResponse, StdError, StdResult, HumanAddr,
};
...
pub fn whitelist_collateral(
    ...
    let overseer = deps.api.human_address(&config.overseer)?;
    query_collateral_whitelist_info(&deps.querier, overseer, collateral_token)
        .map_err(|_| {
    ...
    let res = HandleResponse {
        attributes: vec![
            attr("action", "whitelist_collateral"),
        ],
        messages: vec![],
        data: None,
    };
...
pub fn update_collateral_info(
    ...
    let res = HandleResponse {
        attributes: vec![
            attr("action", "update_collateral_info"),
        ],
        messages:  vec![],
        data: None,
    };
