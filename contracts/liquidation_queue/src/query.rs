...
use cosmwasm_std::{CanonicalAddr, Deps, StdResult, Uint128, HumanAddr};
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        ...
        overseer: deps.api.human_address(&config.overseer)?.to_string(),
    };
...
pub fn query_bid(deps: Deps, bid_idx: Uint128) -> StdResult<BidResponse> {
    ...
    Ok(BidResponse {
        idx: bid.idx,
        collateral_token: deps.api.human_address(&bid.collateral_token)?.to_string(),
        bidder: deps.api.human_address(&bid.bidder)?.to_string(),
        amount: bid_amount,
...
pub fn query_collateral_info(
    ...
    Ok(CollateralInfoResponse {
        collateral_token: deps.api.human_address(&collateral_token_raw)?.to_string(),
